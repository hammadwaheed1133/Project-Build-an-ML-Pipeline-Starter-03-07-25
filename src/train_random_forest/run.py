...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # DEBUG LINES - ADD THESE TO SEE WHAT'S HAPPENING
    print(f" DEBUG: Pipeline steps: {sk_pipe.steps}")
    print(f" DEBUG: Number of steps: {len(sk_pipe.steps)}")
    print(f" DEBUG: Pipeline type: {type(sk_pipe)}")
    if len(sk_pipe.steps) > 0:
        print(f" DEBUG: First step: {sk_pipe.steps[0]}")
        print(f" DEBUG: Second step: {sk_pipe.steps[1] if len(sk_pipe.steps) > 1 else 'None'}")
    else:
        print(" DEBUG:  PIPELINE HAS NO STEPS!")
